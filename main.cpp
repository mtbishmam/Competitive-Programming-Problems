    ...
    for (int Ti = 1; Ti <= T; Ti++)
    {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                ...
                for (int k = 0; k < m - 1; k++) {
                    cmn[i][j] = min(cmn[i][j], abs(a[i][k + 1] - a[j][k]));
                }
            }
        }
        int ans = 0, first = -1;
        function<int(int, int)> f = [&](int prev, int cur) {
            if (__builtin_popcount(cur) == n) return cmn[first][prev];
            int& ret = dp[prev][cur];
            if (~ret) return ret;
            ...
            for (int j = 0; j < n; j++) {
                if (!(cur & (1 << j)))
                    ret = max(ret, min(mn[prev][j], f(j, cur | (1 << j))));
            }
            return ret;
        };
        for (int i = 0; i < n; i++) {
